...
export default function HospitalAffiliations({ hospitals, className = "" }) {
  ...
  return (
    <div className={`mx-auto mt-10 w-full rounded-lg bg-white p-2 shadow min-md:p-6 ${className}`}>
      ...
      <div className="">
        {/* Hospital Cards */}
        <div className={`flex overflow-hidden max-sm:flex-col ${hospitals.length === 1 ? "" : ""}`}>
          {visibleHospitals.map((hospital, index) => (
            <div
              key={hospital.name}
              className={`w-full flex-shrink-0 transition-all duration-300 ease-in-out ${
                hospitals.length === 1 ? "min-md:w-full" : "min-md:w-1/2"
              } ${
            ...
            >
              <div className={`h-full w-full overflow-hidden py-4 ${hospitals.length === 1 ? "" : "max-w-[220px]"}`}>
                <p className="mb-1 text-base font-semibold tracking-wide text-gray-500">
                  Hospital Name
                </p>
                ...
                <a
                  ...
                  className="text-lg text-gray-600 min-md:w-50 hover:text-primary underline-offset-2 hover:underline transition-colors"
                >
                  {hospital.address}
                </a>
                <p className="mt-4 mb-1 text-base font-semibold tracking-wide text-gray-500">
                  Phone Number
                </p>
                <a
