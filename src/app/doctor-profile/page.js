...
const page = () => {
  ...
  const info_style = "text-[15px] font-[600] mt-2";
  const main_heading = "text-[20px] font-[600] text-primary mb-5";
  const box_style = "bg-secondary rounded-xl mt-10 max-w-[800px]";
  return (
    <div className="container m-auto">
      ...
      <div>
       ...
       <div>
        ...
        {/* doc details  */}
        <div className={`${box_style} py-8 px-13`}>
          <p className={`${main_heading}`}>Doctor’s Information</p>
          {
            docProfile_Details.doc_details.map((data, key) => (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7 items-center justify-center" key={key}>
                <div>
        ...
        {/* doc location  */}
         ...
         <div className={`${box_style} flex justify-between items-center p-2`}>   
            ...
            <div>
              <iframe src={`https://www.google.com/maps?q=${encodeURIComponent(docProfile_Details.doc_details[0].clinic_loc)}&output=embed`} width="315" height="152" style={{"border":"0"," border-radius":"9px"}} allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>
            </div>
        </div>
      </div>
      {/* right area */}
